...
static unsigned long next_telemetry_send_time_ms = 1000;
...
static int initializeMqttClient() {
  ...
  mqtt_config.credentials.authentication.certificate_len =
      (size_t)sizeof(IOT_CONFIG_DEVICE_CERT);
  mqtt_config.credentials.authentication.key =
      IOT_CONFIG_DEVICE_CERT_PRIVATE_KEY;
  mqtt_config.credentials.authentication.key_len =
      (size_t)sizeof(IOT_CONFIG_DEVICE_CERT_PRIVATE_KEY);
#else // Using SAS key
  mqtt_config.credentials.authentication.password =
      (const char *)az_span_ptr(sasToken.Get());
#endif
...
static void generateTelemetryPayload() {
  // You can generate the JSON using any lib you want. Here we're showing how to
  // do it manually, for simplicity. This sample shows how to generate the
  // payload using a syntax closer to regular delevelopment for Arduino, with
  // String type instead of az_span as it might be done in other samples. Using
  // az_span has the advantage of reusing the same char buffer instead of
  // dynamically allocating memory each time, as it is done by using the String
  // type below.
  // telemetry_payload =
  //    "{ \"msgCount\": " + String(telemetry_send_count++) + " }";

#if defined(SIMULATED_DATA)
  float temperature = random(20, 30);
  float humidity = random(30, 40);
  float pressure = random(1500, 1700);
...
void setup() {
  ...
  Logger.Info("Going to the deep sleep");
  esp_sleep_enable_timer_wakeup(TIME_TO_SLEEP * uS_TO_S_FACTOR);
  esp_deep_sleep_start();

#else
  if (!bme.begin()) {
    Logger.Info(F("Could not find a valid BME280 sensor, check wiring!"));
  } else {
    establishConnection();
